...
const LOCATIONS = ["Ismaning", "Unterföhring", "Garching", "Schwabing", "Bogenhausen", "Altbogenhausen", "Oberföhring"];
...
const CRITERIA = {
  ko: [
    ...
    { id: 'location', label: 'Good neighborhood (Schwabing, Ismaning, etc.)', type: 'select', options: LOCATIONS, icon: MapPin },
  ...
  prio1: [
    ...
    { id: 'kitaSpot', label: 'Available daycare spot', icon: Baby }, // NEW
    ...
    { id: 'neighborhood', label: 'Decent neighborhood', icon: Users }, // NEW
  ...
  prio3: [
    { id: 'rooms45', label: 'Bonus: 4.5 rooms', icon: Plus }, // NEW
    { id: 'garden', label: 'Garden', icon: Flower }, // NEW
    { id: 'kitchenLiving', label: 'Open-plan kitchen/living area', icon: Coffee }, // NEW
    { id: 'noComplex', label: 'Not a huge apartment complex', icon: Building }, // NEW
    { id: 'shower', label: 'Walk-in shower', icon: Droplets }, // NEW
...
export default function ApartmentScorer() {
  const [apartments, setApartments] = useState(() => {
    const saved = localStorage.getItem('apartments_v2');
  ...
  useEffect(() => {
    localStorage.setItem('apartments_v2', JSON.stringify(apartments));
  }, [apartments]);
  ...
  const calculateScore = (apt) => {
    ...
    if (!LOCATIONS.some(l => apt.ko.location === l)) koFailures.push(`Location (${apt.ko.location})`);
    ...
    CRITERIA.prio1.forEach(c => {
      ...
      if (apt.prio1 && apt.prio1[c.id]) score += POINTS.prio1;
    });

    // Priority 3
    CRITERIA.prio3.forEach(c => {
      maxScore += POINTS.prio3;
      if (apt.prio3 && apt.prio3[c.id]) score += POINTS.prio3;
    });
  ...
  const handleEdit = (apt) => {
    // Merge with initial state to ensure new criteria fields exist
    const mergedData = {
        ...initialFormState,
        ...apt,
        ko: { ...initialFormState.ko, ...apt.ko },
        prio1: { ...initialFormState.prio1, ...apt.prio1 },
        prio3: { ...initialFormState.prio3, ...apt.prio3 }
    };
        ...
        <div className="max-w-5xl mx-auto px-4 py-4 flex justify-between items-center">
          <div>
            <h1 className="text-xl font-bold text-slate-900 flex items-center gap-2">
              ...
              Apartment Checker V2
            </h1>
            <p className="text-xs text-slate-500">Munich & surroundings • Updated</p>
          </div>
          <button
            onClick={() => {
              setFormData(initialFormState);
                  ...
                  <div className="p-5 flex flex-col md:flex-row gap-6">
                    ...
                    <div className="flex-grow">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                           {CRITERIA.prio1.filter(c => apt.prio1 && apt.prio1[c.id]).map(c => (
                           ...
                           {!isKo && CRITERIA.prio1.filter(c => !apt.prio1 || !apt.prio1[c.id]).slice(0, 3).map(c => (
              ...
              <section className="space-y-4">
                <div>
                  ...
                  <input
                    autoFocus
                    type="text"
                    placeholder="e.g. Dream apartment Schwabing"
